...
public class TileBasedGraphicsChip extends GraphicsChip {
	...
	/**
	 * This class represents a tile in the tile data area. It contains images for a
	 * tile in each of it's three palettes and images that are flipped horizontally
	 * and vertically. The images are only created when needed, by calling
	...
	class GameboyTile {
		...
		/**
		 * True, if the tile's image in the image[] array is a valid representation of
		 * the tile as it appers in video memory.
		 */
		...
		public void allocateImage(int attribs, Component a) {
			source[attribs] = new MemoryImageSource(8 * magnify, 8 * magnify,
					new DirectColorModel(32, 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000), imageData, 0,
					8 * magnify);
		...
		/**
		 * Returns true if this tile does not contian a valid image for the tile with
