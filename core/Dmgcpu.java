...
public class Dmgcpu {
	...
	/**
	 * Enable the breakpoint flag. As breakpoint instruction is used in some games,
	 * this is used to skip over it unless the breakpoint is actually in use
	 */
	 ...
	 * are executed for each horizontal line scanned on the screen. Multiply by 154
	 * to find out how many instructions per frame.
	 */
	...
	public void terminateProcess() {
		 ...
		 * System.out.println("Wating for CPU..."); } catch (InterruptedException e) {
		 * // Nothing } } while (running);
		 */
	}

	/**
	 * Perform a CPU address space read. This maps all the relevant objects into the
	...
	/**
	 * Performs a CPU address space write. Maps all of the relevant object into the
	 * right parts of memory.
	 */
	...
	public final void initiateInterrupts() {
		...
		if ((instrCount % INSTRS_PER_HBLANK) == 0) {
			...
			if (((ioHandler.registers[0xFF] & INT_LCDC) != 0) && ((ioHandler.registers[0x41] & 0x8) != 0)
					&& ((ioHandler.registers[0x40] & 0x80) != 0) && (cline < 0x90)) {
				// System.out.println("** LCDC Int **");
				triggerInterrupt(INT_LCDC);
			}
	...
	public final void execute(int numInstr) {
		...
		for (int r = 0; (r != numInstr) && (!terminate); r++) {
				...
				if (gbcFeatures) {
					if ((ioHandler.registers[0x4D] & 0x01) == 1) {
						...
						if ((newKey1Reg & 0x80) == 0x80) {
							...
							setDoubleSpeedCpu(true);
							newKey1Reg |= 0x80;
							// System.out.println("CAUTION: Game uses double speed CPU, humoungus PC
							// required!");
						}
			...
			case 0x27: // DAA ** This could be wrong! **
				...
				if ((f & F_SUBTRACT) == 0) {
					...
					if ((f & F_CARRY) == 0) {
						...
						if ((upperNibble <= 9) && (lowerNibble <= 0x3) && ((f & F_HALFCARRY) == F_HALFCARRY)) {
							a += 0x06;
						}
						...
						if ((upperNibble >= 0xA) && (lowerNibble <= 0x3) && ((f & F_HALFCARRY) == F_HALFCARRY)) {
							a += 0x66;
							newf |= F_CARRY;
						}
						...
						if ((upperNibble <= 0x3) && (lowerNibble <= 0x3) && ((f & F_HALFCARRY) == F_HALFCARRY)) {
							a += 0x66;
							newf |= F_CARRY;
						}
					...
					if ((f & F_CARRY) == 0) {
						...
						if ((upperNibble <= 0x8) && (lowerNibble >= 0x6) && ((f & F_HALFCARRY) == F_HALFCARRY)) {
							a += 0xFA;
						}
						...
						if ((upperNibble >= 0x6) && (lowerNibble >= 0x6) && ((f & F_HALFCARRY) == F_HALFCARRY)) {
							a += 0x9A;
							newf |= F_CARRY;
						}
			...
			case 0x2F: // CPL A
				...
				short mask = 0x80;
				/*
				 * short result = 0; for (int n = 0; n < 8; n++) { if ((a & mask) == 0) { result
				 * |= mask; } else { } mask >>= 1; }
				 */
				a = (short) ((~a) & 0x00FF);
			...
			case 0x52: // Debug breakpoint (LD D, D)
				// As this insturction is used in games (why?) only break here if the breakpoint
				// is on in the debugger
				if (breakpointEnable) {
					terminate = true;
					System.out.println("- Breakpoint reached");
				} else {
	...
	/**
	 * Output a disassembly of the specified number of instructions starting at the
	 * speicifed address.
	 */
