...
class TCPGameLink extends GameLink implements Runnable {
	...
	public TCPGameLink(Frame parent) {
		...
		try {
			...
			new ModalDialog(parent, "Server startup succeeded!", "Game Link server running",
	...
	public TCPGameLink(Frame parent, String ip) {
		...
		try {
			...
			new ModalDialog(parent, "Connect succeeded!", "Connected to Game Link server.",
			...
			try {
				...
				while ((data != -1) && (!terminate)) { /* This needs to terminate */
					...
					if (initial == 1) {
						...
						// if ((clock & 0x01) != (JavaBoy.unsign(dmgcpu.ioHandler.registers[0x02]) &
						// 0x01)) {
