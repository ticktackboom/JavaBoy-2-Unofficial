...
class IoHandler {
	...
	public void ioWrite(int num, short data) {
		...
		if (num <= 0x4B) {
			// System.out.println("Write of register " + JavaBoy.hexByte(num) + " to " +
			// JavaBoy.hexWord(data) + " at " + JavaBoy.hexWord(dmgcpu.pc));
		}
		...
		case 0x00: // FF00 - Joypad
			...
			// System.out.println("Joypad port = " + JavaBoy.hexByte(data) + " output = " +
			// JavaBoy.hexByte(output) + "(PC=" + JavaBoy.hexWord(dmgcpu.pc) + ")");
			break;
		...
		case 0x02: // Serial
			...
			if (dmgcpu.gameLink != null) { // Game Link is connected to serial port
				...
				if ((registers[0x02] & 0x01) == 1) {
					registers[0x01] = (byte) 0xFF; // when no LAN connection, always receive 0xFF from port. Simulates
													// empty socket.
					if (dmgcpu.running)
